...
//                        locationsPath);
                ...
                registVoice(this, voice -> {
                    if (StringUtil.isEmpty(voice)) return;

                    //收到语音，取消定时器
                    RecognizeWaitTimer.getInstance().cancel();
                });
    ...
    private BroadcastReceiver receiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if ("REEMAN_LAST_MOVTION".equals(action)) {
                ...
                //唤醒
                //只有在唤醒后才会调用startRecognize开始监听语音
                //等待10秒，若无语音则取消监听
                //收到语音voice后，取消RecognizeWaitTimer定时器

                RobotHandler.getInstance().stopSpeek();

                RobotHandler.getInstance().doSpeek("你好，请说", new ISpeakListener() {
                    @Override
                    public void onSpeakOver(SpeechError speechError) {
                        ...
                        //定时10秒，若无语音则取消监听
                        RecognizeWaitTimer.getInstance().startWait(10, () -> {
                            isRecognizing = false;
                            SpeechPlugin.getInstance().stopRecognize();
                            RobotHandler.getInstance().doSpeek("没听到你在说什么");
                        });
                    }
